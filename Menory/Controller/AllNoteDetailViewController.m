...
- (void)viewDidLoad {
  ...
  [self.shareImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(allNoteDetailShareButtonPressed:)]];
    [self.addImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(jumpAddNoteController)]];
    [AllUtils setBackImage:self.myView imageName:@"主页背景.png"];
}

// 点击 按钮 跳转到新笔记页面
- (void)jumpAddNoteController
{
